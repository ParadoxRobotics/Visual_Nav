...
// Robot Navigation stack -> Patch Divisive contrast Normalization pre-processing
...
// Patch DCN-like function
void PatchDCN(cv::Mat Img, cv::Mat& out, std::vector<int> PatchSize, std::vector<int> Padding, float eps){
  ...
  cv::Mat smoothImg;
  GaussianBlur(Img, smoothImg, Size(PatchSize[0], PatchSize[1]), 0, 0);
  // Get the image size
  int H = Img.rows;
  int W = Img.cols;
  ...
  for (int y = 0; y < row; y++){
    for (int x = 0; x < col; x++){
      ...
      cv::Mat smoothPatch = smoothImg(patchROI);
      // Normalize it
      cv::Scalar mean, stddev;
      cv::meanStdDev(smoothPatch, mean, stddev);
      cv::Mat tmpPatch = (patch - mean[0]) / (1 + eps*(stddev[0]+1e-8));
      tmpPatch.copyTo(out);
    }
  }
}
